...
module.exports = function (host) {
	...
	_self.insert = function (tableName, dataToInsert, callback) {
		insertQueryBuilder(tableName, dataToInsert, callback, false);
	}

	/**
	 * REPLACE
	 * Used to build and launch a Mysql replace query
	 * @param {String} tableName
	 * @param {Object} dataToInsert
	 * @param {Function} callback
	 */
	_self.replace = function (tableName, dataToInsert, callback) {
		insertQueryBuilder(tableName, dataToInsert, callback, true);
	}
	...
	/**
	 * INSERT/REPLACE queryBuilder
	 * Used to build insert and replace queries from same logic
	 * @param {String} tableName
	 * @param {Object} dataToInsert
	 * @param {Function} callback
	 * @param {Boolean} isReplace
	 */
	var insertQueryBuilder = function (tableName, dataToInsert, callback, isReplace) {
		if (tableName && dataToInsert) {
			if (Object.size(dataToInsert) > 0) {
				var mysqlQuery = (isReplace ? 'REPLACE' : 'INSERT') + ' INTO `' + tableName + '` (';
				if (!Array.isArray(dataToInsert)) {
					dataToInsert = [dataToInsert];
				}
				var i = 0;
				for (var index in dataToInsert[0]) {
					mysqlQuery += index;
					mysqlQuery += (i + 1 == Object.size(dataToInsert[0]) ? ') ' : ', ');
					i++;
				}
				mysqlQuery += 'VALUES ';
				var j = 0;
				dataToInsert.forEach(dataItem => {
					i = 0;
					mysqlQuery += '(';
					for (index in dataItem) {
						mysqlQuery += typeof (dataItem[index]) == 'string' ? '"' + dataItem[index].replace(/"/g, '\\"') + '"' : dataItem[index];
						mysqlQuery += (i + 1 == Object.size(dataItem) ? ') ' : ', ');
						i++;
					}
					mysqlQuery += (j + 1 == dataToInsert.length ? '' : ', ');
					j++;
				});
				_self.query(mysqlQuery, callback);
			}
